...
```{r, include=FALSE}
load(file = here::here("discussions", "data", "knn_tune.rda"))
```
...
```{r}
# plot cv results for patameter tuning
...
test_roc_auc = roc_curve(train_predict2, habitat, .pred_bathydemersal, .pred_bathypelagic, .pred_benthopelagic, .pred_demersal, .pred_pelagic, '.pred_pelagic-neritic', '.pred_pelagic-oceanic', '.pred_reef-associated' )
```
...
```{r}
test_predict %>% 
  conf_mat(truth = habitat, estimate = )
```
