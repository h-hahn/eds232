...
fish_clean = read_csv(here("discussions", "data", "fish_clean.csv")) %>% 
...
# use these to bake training data set and visualize what preprocessing steps are doing to the data
?prep # trains the processing steps (sometimes model within a model). calls recipe and data youre prepping
?bake 
```
...
```{r}
fish recipe <- recipe(habitat ~ shape_impute + food_impute + shallow_impute + deep impute + vulnerability + length_impute + trophic_level_impute, data = train) %>% # create model recipe
...
```{r}
train_predict <- predict(train_fit, test) %>% # get prediction probabilityes for test
...
test_metrics <- test_predict %>% 
  metrics(habitat, .pred) # ge testing data metrics
