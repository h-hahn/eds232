...
## R
```{r}
kaggle_dat <- read_csv(here::here("labs", "genres_v2.csv"))
...
```{r recipe}
#Preprocess the data
genre_rec <- recipe(genre ~ ., genre_train) %>% 
...
# last_fit() fit on training data, but then also evaluates on the test data
...
```{r tree_vip}
final_tree_fit %>% 
  vip(geom = "col", aesthetics = list(fill = "cornflowerblue", alpha = 0.8)) +
  scale_y_continuous(expand = c(0,0)) +
  theme_minimal()

ggplot() + geom_boxplot(data = predict_data, aes(.pred_class, danceability))
```
