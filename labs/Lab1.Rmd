...
ggplot(pumpkins) +
  ...
  theme_classic() +
  labs(title = "Pumpkin prices throughout the year",
       x = "Day of Year",
       y = "Price")
```
...
* **This plot is not very useful because most people don't know what day of the year is in which month, and it might be more useful to look at patterns over coarser scales such as months. The points seem to be grouped vertically, which I would assume is because there are different varieties of pumpkins, and each variety is "clumped" into similar prices over time.**

How do we make it more useful? To get charts to display useful data, you usually need to group the data somehow. 

* **To make it more useful, we could group the data by month and even look at different averages among different pumpkin varieties.**
...
* **The correlation between package and price is ~0.61.**
...
* **The correlation between pumpkin price and variety is ~-0.86, and the correlation between price and city is ~0.32.**
...
cor(baked_pumpkins$price, baked_pumpkins$city_name)
```

*Question 7:* Which of these three variables is most highly correlated with price? Why might this be? 

* **The variety variable is most highly correlated with price. This is likely because different varieties of pumpkins are different sizes or are of different quality, so the pricing patterns are similar among variety regardless of where they are and when they are sold.**
...
* **Package type does not appear to have a linear relationship to price, especially because package is a categorical variable The price points are vertically grouped and do not follow a linear pattern. The package variable is not a true numeric variable, so it is difficult to capture the relationship between a categorical and numeric variable with a linear relationship.**
